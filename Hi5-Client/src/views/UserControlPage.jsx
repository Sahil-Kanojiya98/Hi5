...
const UserControlPage = () => {
    ...
    const fetchUsers = useCallback(async () => {
        ...
        try {
            setIsLoading(true);
            const response = await searchUsersByKeywordForModeration(debouncedSearchTerm, {
                size: 10,
                page,
            });
    ...
    const changeUserBanUntil = (id, newDate) => {
        const updatedUsers = users.map((user) => {
            if (user.id === id) {
                return { ...user, banUntil: newDate };
            }
            return user;
        });
                    ...
                    <div className="flex flex-col justify-center bg-white dark:bg-black shadow-lg my-0 md:my-4 px-4 rounded-lg w-full">
                        ...
                        <div className="flex flex-col flex-1 space-y-4 mt-0 mb-10 sm:mb-3 min-[450px]:p-4 pb-4 overflow-y-auto hide-scrollbar">
                            ...
                            {users.length > 0 && (
                                <>
                                    {users.map((user) => (
                                        <UserModerationSearchCard key={user.id} user={user} changeUserBanUntill={changeUserBanUntil} />
                                    ))}
                                </>
                            )}
                            {isLoading && users.length === 0 && (
